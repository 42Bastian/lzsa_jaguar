/*
 * expand_block_v1a.c - LZSA1 block decompressor implementation
 *
...
static inline FORCE_INLINE int lzsa_build_literals_len_v1a(const unsigned char **ppInBlock, const unsigned char *pInBlockEnd, unsigned int *nLiterals) {
   unsigned int nByte;
...
static inline FORCE_INLINE int lzsa_build_match_len_v1a(const unsigned char **ppInBlock, const unsigned char *pInBlockEnd, unsigned int *nMatchLen) {
   unsigned int nByte;
   const unsigned char *pInBlock = *ppInBlock;
 ...
 */
int lzsa_decompressor_expand_block_v1a(const unsigned char *pInBlock, int nBlockSize, unsigned char *pOutData, int nOutDataOffset, int nBlockMaxSize) {
...
//->      printf("\n%03x:%02x ",pInBlock-p+5,token);
      ...
      if (nLiterals != LITERALS_RUN_LEN_V1A && (pInBlock + 8) <= pInBlockEnd && pCurOutData < pOutDataFastEnd) {
         memcpy(pCurOutData, pInBlock, 8);
         pInBlock += nLiterals;
         pCurOutData += nLiterals;
      }
      else {
         if (nLiterals == LITERALS_RUN_LEN_V1A) {
            if (lzsa_build_literals_len_v1a(&pInBlock, pInBlockEnd, &nLiterals))
               return -1;
         }
...
//->         printf("(%d)",nMatchOffset);
         ...
         if (pSrc >= pOutData) {
            ...
            if (nMatchLen != MATCH_RUN_LEN_V1A && nMatchOffset >= 8 && pCurOutData < pOutDataFastEnd && (pSrc + 18) <= pOutDataEnd) {
               memcpy(pCurOutData, pSrc, 8);
               memcpy(pCurOutData + 8, pSrc + 8, 8);
               memcpy(pCurOutData + 16, pSrc + 16, 2);
               pCurOutData += (MIN_MATCH_SIZE_V1A + nMatchLen);
            }
            else {
               nMatchLen += MIN_MATCH_SIZE_V1A;
               if (nMatchLen == (MATCH_RUN_LEN_V1A + MIN_MATCH_SIZE_V1A)) {
                  if (lzsa_build_match_len_v1a(&pInBlock, pInBlockEnd, &nMatchLen))
                     return -1;
                  if (nMatchLen == 0)
                     break;
               }
