/*
 * shrink_block_v1a.c - LZSA1 block compressor implementation
 *
 ...
 */
static inline int lzsa_get_literals_varlen_size_v1a(const int nLength) {
   if (nLength < LITERALS_RUN_LEN_V1A) {
      return 0;
   }
   else {
      if (nLength < 256)
         ...
         if (nLength < 245+255+LITERALS_RUN_LEN_V1A)
            return 16;
         else
            return 24;
      }
 ...
 * @param nLength literals length
 */
static inline int lzsa_write_literals_varlen_v1a(unsigned char *pOutData, int nOutOffset, const int nLength) {
   if (nLength >= LITERALS_RUN_LEN_V1A) {
      if (nLength < 256)
         pOutData[nOutOffset++] = nLength - LITERALS_RUN_LEN_V1A;
      else {
         if (nLength < 254+255+LITERALS_RUN_LEN_V1A ) {
            pOutData[nOutOffset++] = 254;
            pOutData[nOutOffset++] = nLength - 254-LITERALS_RUN_LEN_V1A;
         }
         else {
 ...
 *
 * @param nLength encoded match length (actual match length - MIN_MATCH_SIZE_V1A)
 *
 * @return number of extra bits required
 */
static inline int lzsa_get_match_varlen_size_v1a(const int nLength) {
   if (nLength < MATCH_RUN_LEN_V1A) {
      return 0;
   }
   else {
      if ((nLength + MIN_MATCH_SIZE_V1A) < 256)
         return 8;
      else {
         if ((nLength + MIN_MATCH_SIZE_V1A) < 512)
            return 16;
         else
            return 24;
      }
 ...
 * @param nLength encoded match length (actual match length - MIN_MATCH_SIZE_V1A)
 */
static inline int lzsa_write_match_varlen_v1a(unsigned char *pOutData, int nOutOffset, const int nLength) {
   if (nLength >= MATCH_RUN_LEN_V1A) {
      if ((nLength + MIN_MATCH_SIZE_V1A) < 256)
         pOutData[nOutOffset++] = nLength - MATCH_RUN_LEN_V1A;
      else {
         if ((nLength + MIN_MATCH_SIZE_V1A) < 512) {
            pOutData[nOutOffset++] = 239;
            pOutData[nOutOffset++] = nLength + MIN_MATCH_SIZE_V1A - 256;
         }
         else {
            pOutData[nOutOffset++] = 238;
            pOutData[nOutOffset++] = (nLength + MIN_MATCH_SIZE_V1A) & 0xff;
            pOutData[nOutOffset++] = ((nLength + MIN_MATCH_SIZE_V1A) >> 8) & 0xff;
         }
      }
   }
 ...
 */
static void lzsa_optimize_forward_v1a(lzsa_compressor *pCompressor, lzsa_match *pBestMatch, const int nStartOffset, const int nEndOffset, const int nReduce) {
   lzsa_arrival *arrival = pCompressor->arrival - (nStartOffset << ARRIVALS_PER_POSITION_SHIFT_V1A);
   ...
   for (i = (nStartOffset << ARRIVALS_PER_POSITION_SHIFT_V1A); i != ((nEndOffset + 1) << ARRIVALS_PER_POSITION_SHIFT_V1A); i += NARRIVALS_PER_POSITION_V1A) {
      lzsa_arrival* cur_arrival = &arrival[i];
      int j;

      memset(cur_arrival, 0, sizeof(lzsa_arrival) * NARRIVALS_PER_POSITION_V1A);

      for (j = 0; j < NARRIVALS_PER_POSITION_V1A; j++)
         cur_arrival[j].cost = 0x40000000;
   }

   arrival[nStartOffset << ARRIVALS_PER_POSITION_SHIFT_V1A].cost = 0;
   arrival[nStartOffset << ARRIVALS_PER_POSITION_SHIFT_V1A].from_slot = -1;

   for (i = nStartOffset; i != nEndOffset; i++) {
      lzsa_arrival* cur_arrival = &arrival[i << ARRIVALS_PER_POSITION_SHIFT_V1A];
      lzsa_arrival* pDestLiteralSlots = &cur_arrival[1 << ARRIVALS_PER_POSITION_SHIFT_V1A];
      int j, m;

      for (j = 0; j < NARRIVALS_PER_POSITION_V1A && cur_arrival[j].from_slot; j++) {
         ...
         else if (nNumLiterals == LITERALS_RUN_LEN_V1A || nNumLiterals == 256 || nNumLiterals == 512) {
            nCodingChoiceCost += 8;
         }

         for (n = 0; n < NARRIVALS_PER_POSITION_V1A /* we only need the literals + short match cost + long match cost cases */; n++) {
            if (nCodingChoiceCost < pDestLiteralSlots[n].cost ||
               ...
               memmove(&pDestLiteralSlots[n + 1],
                  &pDestLiteralSlots[n],
                  sizeof(lzsa_arrival) * (NARRIVALS_PER_POSITION_V1A - n - 1));
      ...
      const lzsa_match *match = pCompressor->match + ((i - nStartOffset) << MATCHES_PER_INDEX_SHIFT_V1A);
      const int nNumArrivalsForThisPos = j;

      for (m = 0; m < NMATCHES_PER_INDEX_V1A && match[m].length; m++) {
         int nMatchLen = match[m].length;
         const int nMatchOffsetCost = lzsa_get_offset_cost_v1a(match[m].offset);
         ...
         for (k = nStartingMatchLen; k <= nMatchLen; k++) {
            const int nMatchLenCost = lzsa_get_match_varlen_size_v1a(k - MIN_MATCH_SIZE_V1A);
            lzsa_arrival *pDestSlots = &arrival[(i + k) << ARRIVALS_PER_POSITION_SHIFT_V1A];
            ...
            for (j = 0; j < nNumArrivalsForThisPos; j++) {
               ...
               for (n = 0;
                  n < NARRIVALS_PER_POSITION_V1A && pDestSlots[n].from_slot && pDestSlots[n].cost <= nCodingChoiceCost;
                  n++) {
                  if (lzsa_get_offset_cost_v1a(pDestSlots[n].rep_offset) == nMatchOffsetCost) {
                     exists = 1;
                     break;
                  }
               }
               ...
               if (!exists) {
                  ...
                  if (nNonRepMatchIdx >= 0) {
                     memmove(&pDestSlots[nNonRepMatchIdx + 1],
                        ...
                        sizeof(lzsa_arrival) * (NARRIVALS_PER_POSITION_V1A - nNonRepMatchIdx - 1));
   ...
   const lzsa_arrival *end_arrival = &arrival[(i << ARRIVALS_PER_POSITION_SHIFT_V1A) + 0];
   ...
   while (end_arrival->from_slot > 0 && end_arrival->from_pos >= 0 && (end_arrival->from_pos + nStartOffset) < nEndOffset) {
      ...
      end_arrival = &arrival[((end_arrival->from_pos + nStartOffset) << ARRIVALS_PER_POSITION_SHIFT_V1A) + (end_arrival->from_slot - 1)];
   }
}
 ...
 */
static int lzsa_optimize_command_count_v1a(lzsa_compressor *pCompressor, const unsigned char *pInWindow, lzsa_match *pBestMatch, const int nStartOffset, const int nEndOffset) {
   int i;
   int nNumLiterals = 0;
   int nDidReduce = 0;
   ...
   for (i = nStartOffset; i < nEndOffset; ) {
      ...
      if (pMatch->length == 0 &&
         (i + 1) < nEndOffset &&
         pBestMatch[i + 1].length >= MIN_MATCH_SIZE_V1A &&
         pBestMatch[i + 1].length < MAX_VARLEN &&
         pBestMatch[i + 1].offset &&
         i >= pBestMatch[i + 1].offset &&
         (i + pBestMatch[i + 1].length + 1) <= nEndOffset &&
         !memcmp(pInWindow + i - (pBestMatch[i + 1].offset), pInWindow + i, pBestMatch[i + 1].length + 1)) {
         const int nCurLenSize = lzsa_get_match_varlen_size_v1a(pBestMatch[i + 1].length - MIN_MATCH_SIZE_V1A);
         const int nReducedLenSize = lzsa_get_match_varlen_size_v1a(pBestMatch[i + 1].length + 1 - MIN_MATCH_SIZE_V1A);
      ...
      if (pMatch->length >= MIN_MATCH_SIZE_V1A) {
         if (pMatch->length <= 9 /* Don't waste time considering large matches, they will always win over literals */ &&
            ...
            while (nNextIndex < nEndOffset && pBestMatch[nNextIndex].length < MIN_MATCH_SIZE_V1A) {
               nNextLiterals++;
               nNextIndex++;
            }
            ...
            if ((8 /* token */ + lzsa_get_literals_varlen_size_v1a(nNumLiterals) + ((pMatch->offset <= 256) ? 8 : 16) /* match offset */ + lzsa_get_match_varlen_size_v1a(pMatch->length - MIN_MATCH_SIZE_V1A) +
               8 /* token */ + lzsa_get_literals_varlen_size_v1a(nNextLiterals)) >=
               (8 /* token */ + (pMatch->length << 3) + lzsa_get_literals_varlen_size_v1a(nNumLiterals + pMatch->length + nNextLiterals))) {
               /* Reduce */
         ...
         if ((i + pMatch->length) < nEndOffset && pMatch->offset && pMatch->length >= MIN_MATCH_SIZE_V1A &&
            pBestMatch[i + pMatch->length].offset &&
            pBestMatch[i + pMatch->length].length >= MIN_MATCH_SIZE_V1A &&
            ...
            int nCurPartialSize = lzsa_get_match_varlen_size_v1a(pMatch->length - MIN_MATCH_SIZE_V1A);
            nCurPartialSize += 8 /* token */ + lzsa_get_literals_varlen_size_v1a(0) + ((pBestMatch[i + pMatch->length].offset <= 256) ? 8 : 16) /* match offset */ + lzsa_get_match_varlen_size_v1a(pBestMatch[i + pMatch->length].length - MIN_MATCH_SIZE_V1A);

            const int nReducedPartialSize = lzsa_get_match_varlen_size_v1a(pMatch->length + pBestMatch[i + pMatch->length].length - MIN_MATCH_SIZE_V1A);
 ...
 */
static int lzsa_get_compressed_size_v1a(lzsa_compressor *pCompressor, lzsa_match *pBestMatch, const int nStartOffset, const int nEndOffset) {
   int i;
   int nNumLiterals = 0;
   ...
   {
      const int nCommandSize = 8 /* token */ + lzsa_get_literals_varlen_size_v1a(nNumLiterals) + (nNumLiterals << 3);
 ...
 */
static int lzsa_write_block_v1a(lzsa_compressor *pCompressor, lzsa_match *pBestMatch, const unsigned char *pInWindow, const int nStartOffset, const int nEndOffset, unsigned char *pOutData, const int nMaxOutDataSize) {
   int i;
   int nNumLiterals = 0;
   ...
   for (i = nStartOffset; i < nEndOffset; ) {
      ...
      if (pMatch->length >= MIN_MATCH_SIZE_V1A) {
         ...
         nOutOffset = lzsa_write_literals_varlen_v1a(pOutData, nOutOffset, nNumLiterals);
 ...
 * @return size of compressed data in output buffer, or -1 if the data is uncompressible
 */
static int lzsa_write_raw_uncompressed_block_v1a(lzsa_compressor *pCompressor, const unsigned char *pInWindow, const int nStartOffset, const int nEndOffset, unsigned char *pOutData, const int nMaxOutDataSize) {
   int nNumLiterals = nEndOffset - nStartOffset;
   const int nTokenLiteralsLen = (nNumLiterals >= LITERALS_RUN_LEN_V1A) ? LITERALS_RUN_LEN_V1A : nNumLiterals;
   int nOutOffset = 0;

   int nCommandSize = 8 /* token */ + lzsa_get_literals_varlen_size_v1a(nNumLiterals) + (nNumLiterals << 3) + 4;
   if ((nOutOffset + (nCommandSize >> 3)) > nMaxOutDataSize)
      return -1;
 ...
 * @return size of compressed data in output buffer, or -1 if the data is uncompressible
 */
int lzsa_optimize_and_write_block_v1a(lzsa_compressor *pCompressor, const unsigned char *pInWindow, const int nPreviousBlockSize, const int nInDataSize, unsigned char *pOutData, const int nMaxOutDataSize) {
   ...
   lzsa_optimize_forward_v1a(pCompressor, pCompressor->best_match - nPreviousBlockSize, nPreviousBlockSize, nPreviousBlockSize + nInDataSize, 0 /* reduce */);

   int nDidReduce;
   int nPasses = 0;
   do {
      nDidReduce = lzsa_optimize_command_count_v1a(pCompressor, pInWindow, pCompressor->best_match - nPreviousBlockSize, nPreviousBlockSize, nPreviousBlockSize + nInDataSize);
      nPasses++;
   } while (nDidReduce && nPasses < 20);

   nBaseCompressedSize = lzsa_get_compressed_size_v1a(pCompressor, pCompressor->best_match - nPreviousBlockSize, nPreviousBlockSize, nPreviousBlockSize + nInDataSize);
   ...
   if (nBaseCompressedSize > 0 && nInDataSize < 65536) {
      ...
      lzsa_optimize_forward_v1a(pCompressor, pCompressor->improved_match - nPreviousBlockSize, nPreviousBlockSize, nPreviousBlockSize + nInDataSize, 1 /* reduce */);

      nPasses = 0;
      do {
         nDidReduce = lzsa_optimize_command_count_v1a(pCompressor, pInWindow, pCompressor->improved_match - nPreviousBlockSize, nPreviousBlockSize, nPreviousBlockSize + nInDataSize);
         nPasses++;
      } while (nDidReduce && nPasses < 20);

      nReducedCompressedSize = lzsa_get_compressed_size_v1a(pCompressor, pCompressor->improved_match - nPreviousBlockSize, nPreviousBlockSize, nPreviousBlockSize + nInDataSize);
   ...
   nResult = lzsa_write_block_v1a(pCompressor, pBestMatch, pInWindow, nPreviousBlockSize, nPreviousBlockSize + nInDataSize, pOutData, nMaxOutDataSize);
   if (nResult < 0 && (pCompressor->flags & LZSA_FLAG_RAW_BLOCK)) {
      nResult = lzsa_write_raw_uncompressed_block_v1a(pCompressor, pInWindow, nPreviousBlockSize, nPreviousBlockSize + nInDataSize, pOutData, nMaxOutDataSize);
   }
